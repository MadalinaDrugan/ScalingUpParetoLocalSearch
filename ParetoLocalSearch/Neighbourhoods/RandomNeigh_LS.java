...
public class RandomNeigh_LS implements SetNeighbourhoods {
    ...
    public RandomNeigh_LS(Stack<Iterator_Solutions_QAPs> scal){
    ...
    @Override
    public void restart(){
        currentScal = chooseNeighbours(); //r.nextInt(neigh.size());
    }
    ...
    @Override public RandomNeigh_LS clone(){
        RandomNeigh_LS newS = new RandomNeigh_LS(neigh);
        return newS;
    }
    ...
    @Override public boolean equals(Object o){
        ...
        if(neigh.size() != ((RandomNeigh_LS) o).neigh.size()) {
            return false;
        }
        for(int i=0;i<neigh.size();i++){
              boolean eq = false;
              for(int j = 0; j < neigh.size(); j++){
                if(neigh.get(i).equals(((RandomNeigh_LS)o).neigh.get(i))){
                  eq = true;
                }
              }
              if(!eq) {
